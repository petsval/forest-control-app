...
def layout(title, body, msg='', admin_nav=False):
    message = f'<div class="flash">{esc(msg)}</div>' if msg else ''
    if admin_nav:
        nav = '<nav><a href="/">Koond</a><a href="/vedukamees">Vedukamehe sisestus</a><a href="/admin/import">PRD import</a><a href="/admin/masinad">Masinad ja juhid</a><a href="/admin/lingid">Lingid</a><a href="/admin/jaotus">Tööjaotus</a><a href="/admin/entries">Kanded</a><a href="/admin/drivers">Juhi raport</a><a href="/admin/lisatoode">Lisatööd</a><a href="/admin/export/excellent">Excellent eksport</a><a href="/logout">Logi välja</a></nav>'
    else:
        nav = '<nav class="driver-nav"><strong>Metsatöö sisestus</strong></nav>'
...
<form method="post" action="/login">
  ...
  <button type="submit">Logi sisse</button>
        ...
        self.send_html('Kontori sisselogimine', body, msg, public=True)
    ...
    def send_html(self, title, body, msg='', public=False):
        data = layout(title, body, msg, admin_nav=(self.is_admin() and not public))
...
<form method="get" action="/vedukamees"><label>Juht</label><select name="driver_id" required>{opts(drivers, 'name', True)}</select><button type="submit">Ava minu langid</button></form></div>'''
        if not selected:
            self.send_html('Vedukamehe sisestus', driver_pick, msg, public=True)
            return
        if not stands:
            ...
            self.send_html('Vedukamehe sisestus', body, msg, public=True)
            return
...
<hr><h2>Paranda enda kannet</h2><p class="muted">Parandus jääb logisse.</p><p><a class="button" href="/vedukamees/minu?driver_id={int(selected['id'])}">Näita minu kandeid</a></p><p><a href="/vedukamees">Vali teine juht</a></p></div>'''
    ...
    def page_my_entries(self, msg='', qs=None):
        ...
        self.send_html('Minu kanded', body, msg, public=True)
...
<p><a href="/vedukamees/minu?driver_id={r['driver_id']}">Tagasi minu kannetesse</a></p></div>'''
        self.send_html('Paranda kanne', body, msg, public=True)
...
def run():
    ...
    print(f'Metsatöö kontroll PILV v15 turvaline töötab pordil {PORT}')
    httpd.serve_forever()
